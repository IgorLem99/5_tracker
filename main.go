...
func (r Running) Calories() float64 {
	return ((CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) * float64(r.Weight) * r.Duration.Hours())
}
...
func (w Walking) Calories() float64 {
	speed := w.meanSpeed() * KmHInMsec // конвертация средней скорости из км/ч в м/с
	return ((CaloriesWeightMultiplier*float64(w.Weight) + (speed*speed)/(w.Height)*CaloriesSpeedHeightMultiplier*float64(w.Weight)) * w.Duration.Hours() * float64(MinInHours))
}
